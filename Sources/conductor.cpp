...
Conductor::Conductor(std::string serial_port_name)
{
    ...
    esp_port->async_read_some(asio::buffer(serial_read_buffer), boost::bind(&Conductor::serial_read_handler, this, asio::placeholders::error, asio::placeholders::bytes_transferred));

    // Setup timer
    timer = new asio::high_resolution_timer(io, std::chrono::steady_clock::now() + std::chrono::milliseconds(control_loop_period_ms));
    timer->async_wait(boost::bind(&Conductor::timer_handler, this, asio::placeholders::error));
...
void Conductor::serial_read_handler(const boost::system::error_code& error, std::size_t bytes_transferred)
{
    if(!error)
    {
        ...
        while(find_first_set(serial_read_concat, start, end)) {
            ...
            // Parse it
            auto start_it = serial_read_concat.begin() + start;
            auto start_it = serial_read_concat.begin() + end;
            parse_packet(start_it, end_it);
            
            // Now erase this json packet from serial_read_concat
            serial_read_concat.erase(start_it, end_it+1); // removes the \n as well
        }
...
void Conductor::parse_packet(const std::iterator &start, const std::iterator &end)
{
    // Could copy into a new vector for convenience
    const std::vector<char> cut_vec(start, end);
...
void Conductor::timer_handler(const boost::system::error_code& error)
{
    ...
    if(!error)
    {
        ...
        esp_port->async_write_some( asio::buffer(write_str), boost::bind(&Conductor::serial_write_handler, this, asio::placeholders::error, asio::placeholders::bytes_transferred) );
...
void Conductor::parseAltitude(json &alt_obj)
{
            ...
            && alt_obj.contains("status") )
    {
        ...
        altData.timePc_ms = (int)std::chrono::steady_clock::now();
